...
## LCP > 4 seconds (Poor Performance)
...
resource "newrelic_nrql_alert_condition" "health_check_failure" {
  ...
  description     = "Alerts when health check endpoint returns non-200 status"
  enabled         = true
  violation_time_limit_seconds = 3600
  ...
  critical {
    ...
    threshold_duration_seconds = 180
  }
